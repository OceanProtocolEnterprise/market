...
export default function ComputeWizard({
  ...
  // Use props if available, otherwise use context data
  const asset = propAsset || contextAsset
  const accountId = propAccountId || contextAccountId
  const signer = propSigner || contextSigner

  // For context usage, we need to find the compute service and access details
  ...
  const service =
    ...
    (computeServiceIndex >= 0
      ? asset?.credentialSubject?.services[computeServiceIndex]
      : undefined)
  const accessDetails =
    propAccessDetails ||
    (asset && computeServiceIndex >= 0
      ? asset.accessDetails[computeServiceIndex]
      : undefined)

  // Use defaults for missing props in context mode
  const dtBalance = propDtBalance || '0'
  const file = propFile || null
  const isAccountIdWhitelisted = propIsAccountIdWhitelisted || false
  const fileIsLoading = propFileIsLoading || false
  const consumableFeedback = propConsumableFeedback || ''

  console.log('ComputeWizard resolved data:', {
    asset: asset ? `${asset.id.substring(0, 10)}...` : 'undefined',
    service: service ? service.id : 'undefined',
    accessDetails: accessDetails ? 'present' : 'undefined',
    accountId,
    signer: signer ? 'present' : 'undefined',
    fromContext: !propAsset,
    contextLoading,
    contextError
  })
  ...
  const [algoOrderPriceAndFees, setAlgoOrderPriceAndFees] =
    useState<OrderPriceAndFees>()
  const [datasetOrderPrice, setDatasetOrderPrice] = useState<string>('0')
  const [algoOrderPrice, setAlgoOrderPrice] = useState<string>('0')
  const [c2dPrice, setC2dPrice] = useState<string>('0')
  ...
  const initializeComputeEnvironment = useCallback(async () => {
    if (!service?.serviceEndpoint || !asset?.credentialSubject?.chainId) return
  ...
  // Critical: Algorithm/Dataset fetching based on asset type
  useEffect(() => {
    if (!asset || !service || isUnsupportedPricing) return
    if (asset.credentialSubject?.metadata.type === 'algorithm') {
      // Algorithm → Dataset flow: fetch compatible datasets
      getAlgorithmDatasetsForCompute(
        ...
        setDatasets(datasetLists || [])
        if (datasetLists && datasetLists.length > 0) {
          setDatasetList(datasetLists)
        }
      })
    } else {
      // Dataset → Algorithm flow: fetch compatible algorithms
  ...
  // Critical: Set pricing availability and order transaction validity
  useEffect(() => {
  ...
  // Calculate pricing for wizard steps
  const calculatePricing = useCallback(
    async (formValues: FormComputeData) => {
      if (!asset || !service || !formValues.algorithm) return

      try {
        setIsRequestingPrice(true)

        // Determine if this is algorithm or dataset flow
        const isAlgorithmAsset =
          asset.credentialSubject?.metadata.type === 'algorithm'

        if (isAlgorithmAsset) {
          // Algorithm flow: calculate dataset prices
          const selectedDatasets = formValues.dataset || []
          let totalDatasetPrice = new Decimal(0)

          for (const datasetId of selectedDatasets) {
            // This would need to fetch the actual dataset asset and calculate its price
            // For now, we'll use a placeholder
            totalDatasetPrice = totalDatasetPrice.add(new Decimal('1'))
          }

          setDatasetOrderPrice(totalDatasetPrice.toString())
          setAlgoOrderPrice(accessDetails?.price || '0')
        } else {
          // Dataset flow: calculate algorithm price
          setDatasetOrderPrice(accessDetails?.price || '0')
          if (formValues.algorithm) {
            // Calculate algorithm price based on selected algorithm
            setAlgoOrderPrice(formValues.algorithm.price || '0')
          }
        }

        // Calculate C2D resource pricing
        if (
          formValues.computeEnv &&
          allResourceValues[formValues.computeEnv.id]
        ) {
          const resources = allResourceValues[formValues.computeEnv.id]
          setC2dPrice(resources.price?.toString() || '0')
        }

        // Calculate total prices (simplified version)
        const datasetPriceDecimal = new Decimal(datasetOrderPrice || 0)
        const algoPriceDecimal = new Decimal(algoOrderPrice || 0)
        const c2dPriceDecimal = new Decimal(c2dPrice || 0)
        const total = datasetPriceDecimal
          .add(algoPriceDecimal)
          .add(c2dPriceDecimal)
  ...
  // Transform wizard form data to format expected by onSubmit
  const transformFormData = (
    wizardData: FormComputeData
  ): ComputeDatasetForm => {
    return {
      algorithm: wizardData.algorithm,
      dataset: wizardData.dataset || [],
      computeEnv: wizardData.computeEnv,
      dataServiceParams: wizardData.dataServiceParams || {},
      algoServiceParams: wizardData.algoServiceParams || {},
      algoParams: wizardData.algoParams || {},
      termsAndConditions: wizardData.termsAndConditions,
      acceptPublishingLicense: wizardData.acceptPublishingLicense
    }
  }

  const onSubmitWrapper = async (values: FormComputeData) => {
  ...
  // Set initial loading to false once asset is available
  useEffect(() => {
    console.log('ComputeWizard: asset check', {
      asset: !!asset,
      assetId: asset?.id,
      isLoading,
      contextLoading
    })
    if (asset?.id && !contextLoading) {
      console.log('ComputeWizard: setting isLoading to false')
      setIsLoading(false)
    } else {
      console.log(
        'ComputeWizard: asset or asset.id is missing, or context still loading'
      )
    }
  }, [asset, isLoading, contextLoading])

  // Simple pricing trigger effect (this will be improved later)
  useEffect(() => {
    if (asset && service && algorithms.length > 0) {
      // Calculate initial pricing with default values
      const mockFormValues: FormComputeData = {
        user: { stepCurrent: 1, chainId: 100, accountId: accountId || '' },
        algorithm: algorithms[0],
        dataset: [],
        computeEnv: computeEnvs?.[0] || null,
        cpu: 1,
        gpu: 0,
        ram: 1024,
        disk: 1024,
      ...
      calculatePricing(mockFormValues)
    }
  }, [asset, service, algorithms, computeEnvs, accountId, calculatePricing])

  console.log('ComputeWizard render:', {
    asset: !!asset,
    assetId: asset?.id,
    isLoading,
    contextLoading,
  ...
  if (!asset?.id || isLoading || contextLoading) {
    return (
  ...
  if (error || contextError) {
    return (
      <div className={styles.container}>
        <h2>Error</h2>
        <p className={styles.error}>{error || contextError}</p>
      </div>
    )
  }
